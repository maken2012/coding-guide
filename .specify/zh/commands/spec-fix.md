...
生成修复方案后，进入等待审批模式：
- 输出：📄 修复方案待审核: file:///<绝对路径>/.specify/specs/<feature_id>/fix-plan.html
- 等待用户确认方案（或直接口头确认）
- 确认后进入步骤 3 实施
...
```json
{"id":1,"ts":"<ISO时间>","issue":"<问题描述>","severity":"<simple|medium|complex>","phase":"<发生阶段>","root_cause":"<根因>","files_changed":["path1","path2"],"fix_summary":"<修复摘要>","has_plan":true,"tests":["<测试命令>"],"regression":"<回归测试结果>","status":"fixed"}
```
