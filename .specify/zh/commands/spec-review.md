...
**以需求和设计为基准审查代码**，但按变更范围按需加载：

| 文件 | 用途 | 必读？ |
|------|------|-------|
| `spec.html` | 需求覆盖完整性 | ✅ 始终 |
| `detail.html` | 业务规则是否实现 | ✅ 始终 |
| `*.feedback.json` | 用户决策是否执行 | ✅ 始终 |
| `design/*.html` | 实现是否偏离设计 | 🔍 按变更涉及的领域加载 |
...
### 4.1 反应式等待审批
生成文档后，进入轮询等待模式：
- 使用 ScheduleWakeup 每 60-120 秒检查 `review.feedback.json` 中 `review.verdict` 的值
- 如果 `verdict` 为 `null`，继续等待，输出：⏳ 等待审批: file:///.../review.html
