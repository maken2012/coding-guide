...
### 5.1 反应式等待审批
生成文档后，进入轮询等待模式：
- 使用 ScheduleWakeup 每 60-120 秒检查 `tasks.feedback.json` 中 `review.verdict` 的值（同时可通过 `curl -s http://localhost:8421/api/phases/<feature_id>` 获取阶段状态）
- 如果 `verdict` 为 `null`，继续等待，输出：⏳ 等待审批: http://localhost:8421/specs/<feature_id>/tasks.html
...
📄 实施计划: http://localhost:8421/specs/<feature_id>/plan.html
📄 任务清单: http://localhost:8421/specs/<feature_id>/tasks.html
